...
BEGIN
  ...
  -- html for all snaps
    ...
    dbms_output.put_line('spool full.html');
    dbms_output.put_line('select output from table(dbms_workload_repository.awr_report_html('||c_dbid||','||c_inst_num||','||c_start_snap_id||','||c_end_snap_id||','||c_awr_options||'));');
