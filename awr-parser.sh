...
process_awr_report() {
		...
		if [ "$AWR_SECTION" = "AWRProfile" ]; then
			...
			case ${AWRLINE[@]:0:2} in
				...
				"Buffer  Hit")
					...
					;;
                                "Library Hit")
                                        # Found library hit and soft parse ratios
                                        LIBRARY_HIT_RATIO=${AWRLINE[3]}
                                        echodbg "LIBRARY_HIT_RATION = ${ENDCHAR}${LIBRARY_HIT_RATION}${ENDCHAR}"
                                        # The call to tr is necessary to remove the trailing return character
                                        SOFT_PARSE_RATIO=$(echo ${AWRLINE[7]} | tr -d '\r')
                                        echodbg "SOFT_PARSE_RATIO = ${ENDCHAR}${SOFT_PARSE_RATIO}${ENDCHAR}"
                                        continue
                                        ;;
                                "Execute to Parse")
                                        EXECUTE_PARSE_RATIO=${AWRLINE[4]}
                                        echodbg "EXECUTE_PARSE_RATIO = ${ENDCHAR}${EXECUTE_PARSE_RATIO}${ENDCHAR}"
                                        LATCH_HIT_RATIO=$(echo ${AWRLINE[8]} | tr -d '\r')
                                        echodbg "LATCH_HIT_RATIO = ${ENDCHAR}${LATCH_HIT_RATIO}${ENDCHAR}"
					continue
                                        ;;
                                "Parse CPU to Parse Elapsd")
                                        PARSE_CPU_PARSE=${AWRLINE[6]}
                                        echodbg "PARSE_CPU_PARSE = ${ENDCHAR}${PARSE_CPU_PARSE}${ENDCHAR}"
                                        NON_PARSE_CPU=$(echo ${AWRLINE[10]} | tr -d '\r')
                                        echodbg "NON_PARSE_CPU = ${ENDCHAR}${NON_PARSE_CPU}${ENDCHAR}"
					continue
                                        ;;
				"Instance Efficiency")
				...
				"Buffer Hit")
					...
					;;
                                "Library Hit")
                                        # Found library hit and soft parse ratios
                                        LIBRARY_HIT_RATIO=${AWRLINE[3]}
                                        echodbg "LIBRARY_HIT_RATIO = ${ENDCHAR}${LIBRARY_HIT_RATIO}${ENDCHAR}"
                                        # The call to tr is necessary to remove the trailing return character
                                        SOFT_PARSE_RATIO=$(echo ${AWRLINE[7]} | tr -d '\r')
                                        echodbg "SOFT_PARSE_RATIO = ${ENDCHAR}${SOFT_PARSE_RATIO}${ENDCHAR}"
                                        echovrb "End of Instance Efficiency section found at line $ROWCNT"
	...
	## CALCULO DE %CPU DEL HOST
	CPU_RATIO=`echo "scale=2;$AWR_DB_CPU*100/($DB_NUM_CPUS*$AWR_ELAPSED_TIME*60)" | bc -l`
...
print_report_info() {
	...
	echoprt " Histogram log file parallel write   >1s = $HGRM_LFPW_GT1S"
        echoprt "                       Library Hit Ratio = $LIBRARY_HIT_RATIO"
        echoprt "                        Soft Parse Ratio = $SOFT_PARSE_RATIO"
        echoprt "                  Execute to Parse Ratio = $EXECUTE_PARSE_RATIO"
        echoprt "                         Latch Hit Ratio = $LATCH_HIT_RATIO"
        echoprt "         Parse CPU to Parse Elapsd Ratio = $PARSE_CPU_PARSE"
        echoprt "                     Non-Parse CPU Ratio = $NON_PARSE_CPU"
        echoprt "                            DB CPU Ratio = $CPU_RATIO"
}
...
while (( "$#" )); do
	...
	if [ -r "$1" ]; then
		...
		if [ "$CHECK_HTML" -gt "0" ]; then
			...
			unset HGRM_DFSR_1MS HGRM_DFSR_2MS HGRM_DFSR_4MS HGRM_DFSR_8MS HGRM_DFSR_16MS HGRM_DFSR_32MS HGRM_DFSR_LT1S HGRM_DFSR_GT1S HGRM_LFPW_1MS HGRM_LFPW_2MS HGRM_LFPW_4MS HGRM_LFPW_8MS HGRM_LFPW_16MS HGRM_LFPW_32MS HGRM_LFPW_LT1S HGRM_LFPW_GT1S LIBRARY_HIT_RATIO SOFT_PARSE_RATIO EXECUTE_PARSE_RATIO LATCH_HIT_RATIO PARSE_CPU_PARSE NON_PARSE_CPU CPU_RATIO
